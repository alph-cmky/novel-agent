...
基于 LangGraph 的开源多 Agent 小说写作框架。递归自进化流水线，内建自动迭代与人工审批机制，面向长篇创作，提供 CLI / Web SPA 两种交互方式。
...
│                                                     │
│  Orchestrator → [ Writer                            │
│    → QualityGate(确定性, 通过→可跳过审查)            │
│    → Editor → Continuity → Worldbuilding           │
...
graph TD
    ...
    W --> QG{QualityGate<br/>确定性硬性检查}
    QG -->|通过+跳过审查| WB2[Worldbuilding<br/>实体提取]
    QG -->|需要审查| E[Editor<br/>5维审查]
    E --> C[Continuity<br/>一致性审计]
    C --> WB1[Worldbuilding<br/>进化内实体提取]
    WB1 --> EO[EvolutionOrchestrator<br/>元评估+Delta+终止判断]
    EO -->|继续迭代| W
    EO -->|终止| SB[SelectBest<br/>选择最优版本]
    SB --> WB2
    WB2 --> H[Human Review<br/>interrupt⏸️]
    H -->|批准| DONE((完成))
    H -->|拒绝| W

    style O fill:#4a90d9,color:#fff
    style W fill:#7b4dd3,color:#fff
    style QG fill:#f56c6c,color:#fff
    style E fill:#e6a23c,color:#fff
    style C fill:#e6a23c,color:#fff
    style WB1 fill:#67c23a,color:#fff
    style EO fill:#4a90d9,color:#fff
    style SB fill:#909399,color:#fff
    style WB2 fill:#67c23a,color:#fff
...
| Writer | 章节创作（默认 V2 profile：Canon 优先 + 段落原则）+ search_context 工具 | Quality |
| QualityGate | 确定性硬性检查：篇幅/空内容/大纲覆盖/场景完整性，通过时可跳过 LLM 审查 | —（规则层） |
...
| SelectBest | 硬约束优先 + 多目标选择（Pareto → 综合分兜底），设置最终 draft_content（实际落库在 SSE 层） | —（规则层） |
| Worldbuilding | 实体提取、冲突检测、持久化到 SQLite（进化子图内 + 终选后各运行一次） | Budget |
...
**进化子图** — 每轮 `Writer → Editor → Continuity → Worldbuilding → EvolutionOrchestrator` 产出评估报告，`EvolutionOrchestrator` 对比上轮计算各维度 Delta，生成结构化 `improvement_plan`（focus_dimensions + preserve + avoid）驱动下一轮创作。进化过程中不写 DB（只存 LangGraph checkpoint），结束一次性落库。
...
| 终止状态 | 触发条件 |
|---------|---------|
| `hard_constraint_violation` | 篇幅低于最优版本 85%、一致性错误增加、大纲覆盖率下降、必要事实丢失 |
| `quality_regression` | 单维度暴跌（Δ < -10）、Editor 总分暴跌（Δ < -10）、或综合分低于历史最优 5 分以上 |
| `ceiling` | 所有维度 + Editor + Continuity 均超 90 分（需至少 1 轮迭代） |
| `max_rounds` | 达到最大重写轮次 |
| `converged` | 所有维度 Delta 绝对值低于阈值（默认 3.0） |
| `plateau` | 连续 2 轮所有维度 Delta 均低于阈值 |
...
│   ├── run_service.py   # 写作运行管理（run 生命周期）
